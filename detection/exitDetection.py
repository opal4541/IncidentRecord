...
class ExitClient(object):
    ...
    def send_data(self, frame, exitlicense, exittime):
        sio.emit('exit2server', {
            'exitimage': self._convert_image_to_jpeg(frame),
            'exitlicense': exitlicense,
            'exittime': exittime
        },
                 namespace='/exit')
...
# detectExitLicensePlate
def main(video, server_addr):
    streamer = ExitClient(server_addr).setup()
    ...
    while True:
        ...
        if licenseText != temp:
            ...
            print(exitTime)
            addExitHistory(licenseText, exitTime)
            cv2.imwrite('static/exitVideo/exit.jpg', img)
            streamer.send_data(img, licenseText, exitTime)
...
def addExitHistory(licensePlate, time):
    exitTime = time
    carID = getCarID(licensePlate)

    history = cursor.execute(
        'SELECT TOP 1 HistoryID FROM History WHERE CarID = ? ORDER BY HistoryID DESC',
        carID)
    history = cursor.fetchone()
    cursor.execute('UPDATE History SET ExitTimestamp = ? WHERE HistoryID = ?',
                   exitTime, history[0])
    cursor.commit()
    return True
